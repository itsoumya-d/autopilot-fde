    ...
    def to_visualization(self) -> dict[str, list[dict[str, Any]]]:
        ...
        for process_index, process in enumerate(self.processes.values()):
            ...
            nodes.append({
                "id": process.id,
                "type": "processNode",
                "data": {"label": process.name, "traces": process.metrics.trace_count},
            ...
            for step_index, activity in enumerate(process.activities):
                ...
                nodes.append({
                    "id": node_id,
                    "type": "activityNode",
                    "data": {"label": activity.name, "evidence": activity.evidence},
                    "position": {"x": base_x + step_index * 35, "y": base_y + 110},
                })
            for edge in process.edges:
                edges.append({
                    "id": f"{process.id}:{edge.source}:{edge.target}",
                    "source": f"{process.id}:{edge.source}",
                    "target": f"{process.id}:{edge.target}",
                    "label": f"{edge.probability:.0%}",
                    "data": {"duration_minutes": edge.avg_duration_minutes},
                })
        ...
        return [
            {
                ...
                "source": edge.source,
                "target": edge.target,
                "duration_minutes": edge.avg_duration_minutes,
            }
            for process in self.processes.values()
            for edge in process.edges
            if edge.avg_duration_minutes > threshold_minutes
        ]
