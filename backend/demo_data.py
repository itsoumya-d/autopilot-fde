...
from .models.schema import Channel, ChannelStatus, ChannelType, Message
...
def demo_messages() -> list[Message]:
    ...
    for i, service in enumerate(("AuthGateway", "PaymentService", "DataIngestor", "SearchIndex", "NotificationBus")):
        ...
        add(case, base + 0.8, "elena-dev", "Found root cause in commit hash 4a8f9b: memory leak in connection pool. Assigned to platform team.", "devops")
        ...
        add(case, base + 2.5, "alex-sre", "Telemetry nominal. Post-mortem scheduled. Resolution confirmed and verified with monitoring.", "devops")
    ...
    for i, client in enumerate(("Acme Corp", "Orbit Global", "Juniper Health", "Fintech Labs", "Apex Retail")):
        ...
        add(case, base + 4.5, "noah-support", "Hotfix deployed to production. Tested client export. Fix verified and resolution confirmed.", "support")
    ...
    for i, prospect in enumerate(("Stripe Enterprise", "Snowflake AI", "Palantir Gov", "Databricks Scale")):
        ...
        add(case, base + 4.0, "marcus-vp-sales", "Discount approved with net-30 payment terms clause. Drafted revised quote schedule.", "sales")
    ...
    for i, company in enumerate(("Vercel Partner", "Scale AI", "Mistral Hub", "Anthropic Labs", "Cerebras")):
        ...
        add(case, base + 0.8, "ava-sdr", "Assigned account executive David and demo scheduled on calendar for next Tuesday 2 PM EST.", "sales")
        add(case, base + 1.2, "ava-sdr", "Sent pre-demo questionnaire and customer research briefing to AE.", "sales")
    ...
    for i, vendor in enumerate(("AWS Cloud", "Datadog Telemetry", "Snowflake Warehouse", "Twilio API", "Cloudflare CDN")):
        ...
        add(case, base + 1.5, "lee-ap", "Reconciled the invoice against usage logs. Identified unallocated surge egress bandwidth.", "finance")
        add(case, base + 3.0, "sam-controller", "Payment approval requested from Infrastructure Director for variance.", "finance")
    ...
    for i, name in enumerate(("Sarah Jenkins (SWE)", "Rohan Mehta (Product)", "Chloe Chen (Design)", "Marcus Vance (Sales)")):
        ...
        add(case, base + 3.0, "kyle-it", "IT hardware provisioning requested: MacBook M3 Max + Yubikey + Okta profile configured.", "hr")
        add(case, base + 8.0, "jordan-hr", "Welcome kit dispatched. Onboarding buddy assigned and day-1 calendar invites sent.", "hr")
    ...
    for i, partner in enumerate(("Palantir Foundry", "OpenAI RedTeam", "Cohere Enterprise", "NVIDIA Inception")):
        ...
        add(case, base + 2.0, "rachel-legal", "Triaged contract against company standard fallback terms. Redlined liability cap and governing law.", "legal")
        ...
        add(case, base + 10.0, "rachel-legal", "Counterparty accepted revisions. Final execution copy prepared for CEO signature.", "legal")
    ...
    for i, enterprise in enumerate(("Siemens Cloud", "Boeing Digital", "Target Retail", "Pfizer Labs")):
        ...
        add(case, base + 7.0, "lisa-csm", "Executive renewal presentation delivered. Client confirmed intent to renew 3-year extension.", "cs")
